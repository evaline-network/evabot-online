...
export class ConsiliumEngine {
   ...
   */
  private resolveParticipants(options: ConsiliumRunOptions): ConsiliumParticipant[] {
  ...
  private async runBroadcast(
    ...
    const promises = participants.map(async (p) => {
      ...
      try {
        ...
        const failedTurn: ConsiliumTurn = {
          round: 1,
          participantId: p.id,
          name: p.name || p.id,
          model: p.model,
  ...
  private async runDialogue(
    ...
    for (let round = 1; round <= totalRounds; round++) {
      ...
      const p1Response = await this.client.generateContent(
        p1.model,
      ...
      const p2Response = await this.client.generateContent(
        p2.model,
