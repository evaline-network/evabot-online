...
export class TuiRenderer {
    static pagesDir = path.resolve(process.cwd(), 'pages');
    static loadPageTemplate(cleanHost) {
    ...
    static getRawTemplate(targetDomain) {
    ...
    static getRawTextTemplate(targetDomain) {
        const cleanHost = (targetDomain || '').split(':')[0].toLowerCase().replace(/^www\./, '');
    ...
    static makeBar(pct, total = 10) {
        const safePct = Math.max(0, Math.min(100, pct));
        const filled = Math.round((safePct / 100) * total);
        return '[' + '■'.repeat(filled) + '□'.repeat(total - filled) + ']';
    }
    static renderText(targetDomain) {
        const cleanHost = (targetDomain || '').split(':')[0].toLowerCase().replace(/^www\./, '');
        ...
        if (body) {
            ...
            if (hydrated.includes('<!-- SLOT:TELEMETRY -->')) {
                hydrated = hydrated.replace(/<!-- SLOT:TELEMETRY -->[\s\S]*?<!-- \/SLOT:TELEMETRY -->/g, `<!-- SLOT:TELEMETRY -->\n${telemetryBlock}\n<!-- /SLOT:TELEMETRY -->`);
            }
            ...
            if (hydrated.includes('<!-- SLOT:PROCESS_WATCHER -->')) {
                hydrated = hydrated.replace(/<!-- SLOT:PROCESS_WATCHER -->[\s\S]*?<!-- \/SLOT:PROCESS_WATCHER -->/g, `<!-- SLOT:PROCESS_WATCHER -->\n${procBlock}\n<!-- /SLOT:PROCESS_WATCHER -->`);
            }
            ...
            if (hydrated.includes('<!-- SLOT:LOG_STREAM -->')) {
                hydrated = hydrated.replace(/<!-- SLOT:LOG_STREAM -->[\s\S]*?<!-- \/SLOT:LOG_STREAM -->/g, `<!-- SLOT:LOG_STREAM -->\n${logBlock}\n<!-- /SLOT:LOG_STREAM -->`);
            }
    ...
    static renderHtml(targetDomain) {
