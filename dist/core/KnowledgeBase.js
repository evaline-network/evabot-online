...
export class KnowledgeBase {
        ...
        if (fs.existsSync(sitePath)) {
            ...
            for (const lang of languages) {
                ...
                if (fs.existsSync(langPath)) {
                    await this.loadLanguageDirectory(langPath, lang);
                }
            }
        }
    ...
    async loadLanguageDirectory(dirPath, language) {
    ...
    inferCategory(filePath) {
        ...
        if (lower.includes('about') || lower.includes('pro-nas') || lower.includes('uber'))
            return 'about';
        if (lower.includes('wholesale') || lower.includes('poshuk-partneriv') || lower.includes('poisk-partnerov'))
            return 'wholesale';
    ...
    formatSearchResults(docs, query) {
        if (docs.length === 0) {
