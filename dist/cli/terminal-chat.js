...
import { AnsiColors, TableFormatter, AnsiStreamWriter, formatPrompt, divider, } from '../core/AnsiStreamEngine.js';
const C = AnsiColors;
...
function printStatusBar(session) {
    ...
    console.log(`  ${C.bold}${C.white}EVABOT${C.reset} ${C.green}🟢 ONLINE${C.reset} │ Model: ${C.bold}${C.cyan}${session.getModel()}${C.reset} [${tierBadge}] │ Mode: ${C.bold}${C.brightYellow}${currentMode.toUpperCase()}${C.reset} │ Role: ${C.bold}${C.white}${currentRole}${C.reset}`);
    console.log(`  ${C.gray}Standards: USD ($) & EUR (€) strictly │ 34 Models Active │ Type /help for commands${C.reset}`);
    console.log(divider('─', 80, C.gray) + '\n');
}
/**
