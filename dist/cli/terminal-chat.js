...
import { AnsiColors, TableFormatter, AnsiStreamWriter, MouseTracker, formatPrompt, divider, } from '../core/AnsiStreamEngine.js';
const C = AnsiColors;
...
// Boot Sequence
// ============================================================================
async function runBootSequence(activeModel) {
...
// ============================================================================
function printStatusBar(session) {
    ...
    console.log(`  ${C.bold}${C.white}EVABOT${C.reset} ${C.green}[ONLINE]${C.reset} │ ` +
    ...
    console.log(`  ${C.gray}Session Tokens: ${sessionTotalTokens.toLocaleString()} │ ` +
...
// ============================================================================
function printModelsTable(filterCategory) {
    const models = ModelRegistry.getAllModels();
    const filtered = filterCategory
        ? models.filter((m) => m.category.toLowerCase().includes(filterCategory.toLowerCase()) ||
    ...
    const rows = filtered.map((m) => {
        ...
        if (inPrice.includes('('))
            inPrice = inPrice.split('/')[0].trim();
        if (outPrice.includes('('))
            outPrice = outPrice.split('/')[0].trim();
        rowIdx++;
    ...
    const tableStr = TableFormatter.render(rows, {
        columns: [
...
async function handleConsiliumRun(mode, prompt) {
    ...
    try {
        ...
        for (const turn of result.turns) {
            ...
            if (turn.cost) {
                console.log(`${C.gray}├─ TOKENS: In: ${turn.promptTokens || 0} + Out: ${turn.completionTokens || 0} = ${turn.totalTokens || 0} │ ` +
        ...
        if (result.costSummary) {
            ...
            console.log(`${C.gray}├${'─'.repeat(78)}┤${C.reset}`);
            console.log(`${C.gray}│${C.reset}  ${C.bold}TOTAL AUDIT:${C.reset} ${result.costSummary.totalTokens.toLocaleString()} tokens │ ` +
...
async function main() {
    ...
    rl.on('line', async (line) => {
        ...
        try {
            ...
            console.log(`  ${C.gray}MODEL:${C.reset} ${C.bold}${C.white}${session.getModel()}${C.reset} ${freeBadge} │ ` +
            ...
            if (costEst.isFreeTier) {
                console.log(`  ${C.gray}COMMERCIAL VALUATION:${C.reset} ${C.gray}$${costEst.commercialValueUSD.toFixed(6)} USD │ €${costEst.commercialValueEUR.toFixed(6)} EUR${C.reset}`);
            }
            console.log(`${C.gray}${'─'.repeat(78)}${C.reset}\n`);
        }
        catch (err) {
