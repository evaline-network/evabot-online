...
export class ModelRegistry {
    ...
    static getOpenRouterModels() {
        return COMPLETE_GOOGLE_MODEL_CATALOG.filter((m) => m.category.startsWith('OpenRouter') || m.provider === 'OpenRouter');
    }
    ...
    static calculateCost(modelId, promptTokens, completionTokens) {
        ...
        const formatCost = (val, isFreeFlag, symbol) => {
            if (isFreeFlag)
                return `${symbol}0.00 (100% Free Quota)`;
            if (val === 0)
                return `${symbol}0.00`;
            if (val < 0.0001)
