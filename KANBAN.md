...
> **НОВЫЙ ЭТАП — EvaLine Platform Unification** (см. `docs/platform_unification_plan.{ru,en,uk}.md`):
> объединение 2 серверов (EvaBrain/EvaFace) и 4 доменов в единую платформу
> с общим bundle, конфигом узла и общим backend'ом при полной независимости узлов.

- [ ] **TASK-22:** Фронтенд-рефакторинг: выделить общий TUI-движок из 4 `index.html` в `src/web/` (тема, навигация, живые метрики, анимации, NOCSS).
- [ ] **TASK-23:** Создать формат конфига узла `site.config.json` (id, title, section, modules, colors, apiBase, langs) для каждого из 4 доменов.
- [ ] **TASK-24:** Собрать единый `bundle.js` + `site.js` через esbuild; сформировать `deploy/<domain>/` для каждого узла.
- [ ] **TASK-25:** Graceful degradation — сайты работают как статика (OFFLINE-статус) при недоступности `/api/*` или соседей.
- [ ] **TASK-26:** Добавить OpenAPI 3.0-спецификацию `/api/openapi.json` и UI-доки `/api/docs` в evabot-brain.
- [ ] **TASK-27:** Добавить health по узлам `/api/nodes` в evabot-brain (живой статус всех доменов/сервисов).
- [ ] **TASK-28:** (Опц.) Вынести n8n и voice-сервис под общий API-фасад на Brain.
- [ ] **TASK-29:** Обновить `deploy-sync.sh` для синхронизации общего bundle на все узлы EvaFace + backend на Brain.
- [ ] **TASK-30:** Обновить Caddyfile — общий `import common_api` / `import common_root` вместо дублирования `/api/*` на 4 домена.
- [ ] **TASK-31:** Унифицировать i18n (EN/UK/RU) в едином файле, подключённом к bundle.
- [ ] **TASK-32:** Тест независимости — каждый узел открывается сам по себе при выключенных остальных.
- [ ] **TASK-33:** Мониторинг живого статуса узлов + запись в `/api/worklog`.
...
| **TASK-22** | Общий TUI-движок (рефакторинг фронтенда) | P1 (High) | 🔴 Backlog |
| **TASK-23** | Конфиг узла `site.config.json` (4 домена) | P1 (High) | 🔴 Backlog |
| **TASK-24** | Единый `bundle.js` + `deploy/<domain>/` | P1 (High) | 🔴 Backlog |
| **TASK-25** | Graceful degradation (OFFLINE-статус) | P1 (High) | 🔴 Backlog |
| **TASK-26** | OpenAPI 3.0 `/api/openapi.json` + `/api/docs` | P2 (Medium) | 🔴 Backlog |
| **TASK-27** | Health по узлам `/api/nodes` | P1 (High) | 🔴 Backlog |
| **TASK-28** | Общий API-фасад для n8n/voice | P3 (Low) | 🔴 Backlog |
| **TASK-29** | `deploy-sync.sh` (общий bundle → узлы) | P1 (High) | 🔴 Backlog |
| **TASK-30** | Caddyfile `import common_api`/`common_root` | P1 (High) | 🔴 Backlog |
| **TASK-31** | Единый i18n-файл (EN/UK/RU) | P2 (Medium) | 🔴 Backlog |
| **TASK-32** | Тест независимости узлов | P1 (High) | 🔴 Backlog |
| **TASK-33** | Мониторинг статуса узлов → `/api/worklog` | P2 (Medium) | 🔴 Backlog |